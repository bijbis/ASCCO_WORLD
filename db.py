...
class Influx(Database):
    ...
    def write(self, cols, data, where):
        ...
        if where == 'remote':
            RemoteMonitor(host='123.209.113.35', region='Australia Perth', **raw_data)
            RemoteMonitor.commit()
        elif where == 'energy':
            EnergyMonitor(host='123.209.113.35', region='Australia Perth', **raw_data)
            EnergyMonitor.commit()
